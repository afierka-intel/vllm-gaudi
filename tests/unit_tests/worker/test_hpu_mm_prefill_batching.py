...
# ===========================================================================
# Test default max_prefill_batch_size
# ===========================================================================
class TestDefaultMaxPrefillBatchSize:

    @patch('vllm_gaudi.extension.runtime.get_config')
    def test_mm_model_default_is_4(self, mock_get_config):
        """Multimodal models should default to max_prefill_batch_size=4."""
        ...
        supports_mm = True
        default_max_bs = 4 if supports_mm else 1
        result = with_default(mock_config.VLLM_PROMPT_BS_BUCKET_MAX, default_max_bs)
        assert result == 4

    @patch('vllm_gaudi.extension.runtime.get_config')
    def test_non_mm_model_default_is_1(self, mock_get_config):
        """Non-multimodal models should default to max_prefill_batch_size=1."""
        from vllm_gaudi.extension.utils import with_default
    ...
    @patch('vllm_gaudi.extension.runtime.get_config')
    def test_explicit_env_overrides_mm_default(self, mock_get_config):
        """Explicit VLLM_PROMPT_BS_BUCKET_MAX overrides the MM default."""
        from vllm_gaudi.extension.utils import with_default

        mock_config = MagicMock()
        mock_config.VLLM_PROMPT_BS_BUCKET_MAX = 8
        mock_get_config.return_value = mock_config
